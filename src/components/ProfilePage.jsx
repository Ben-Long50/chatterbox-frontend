...
const ProfilePage = () => {
  ...
  return (
    <div
      className={`row-start-1 mx-auto mb-auto grid w-full max-w-screen-lg grid-cols-2 gap-x-12 gap-y-10 px-4 pb-4 pt-12 max-xl:col-start-1 max-xl:col-end-3 md:gap-y-16 md:pt-20 lg:px-8 lg:pb-8 ${visibility ? 'col-start-2 col-end-3' : 'col-start-1 col-end-3'}`}
    >
      <div className="col-span-2 w-full items-center gap-12 justify-self-center md:flex-row">
        <div>
          <ProfilePic
            username={userInfo.username}
            className="mb-6 ml-auto mr-auto block size-48 shrink-0 bg-gray-200 text-9xl md:float-left md:mr-12 md:size-56 dark:bg-gray-900"
          />
          {!editMode ? (
            <h1 className="text-primary text-center text-6xl font-semibold md:text-left md:text-8xl">
              {userInfo.username}
            </h1>
          ) : (
            <input
        ...
        {!editMode ? (
          <p className="text-secondary mx-6 mt-8 text-left text-2xl md:mx-0">
          ...
          <textarea
            ...
            name="bio"
            onChange={handleChange}
            className="text-primary my-2 h-44 w-full rounded-lg bg-gray-200 p-2 text-2xl dark:bg-gray-800"
            type="text"
